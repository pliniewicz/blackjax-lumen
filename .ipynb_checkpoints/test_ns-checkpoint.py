...
Shared (sampled): theta, log10_Lj
Per-component (sampled): G0_knot, G0_ext
...
#    [theta, log10_Lj, G0_knot, G0_ext]                               #
#                                                                     #
#  theta, log10_Lj are SHARED.                                        #
#  G0 is INDEPENDENT per component.                                   #
# ------------------------------------------------------------------ #
...
# ------------------------------------------------------------------ #
#  MAP finder (gradient-based, for diagnostics)                       #
# ------------------------------------------------------------------ #

def find_map(loglikelihood_fn, x0, bounds_lo, bounds_hi, n_steps=500, lr=0.05):
    """Simple projected gradient ascent to find MAP estimate.

    Works where jax.scipy.optimize.minimize fails due to -inf/NaN
    from out-of-bounds evaluations.
    """
    @jax.jit
    def step(x):
        g = jax.grad(loglikelihood_fn)(x)
        x_new = x + lr * g
        # Project back into bounds (with small margin)
        margin = 1e-6 * (bounds_hi - bounds_lo)
...
for model_id in models:
    ...
    x0 = jnp.array([12.0, 49.0, 10.0, 5.0])  # theta, log10Lj, G0_knot, G0_ext
    map_x, map_ll = find_map(loglikelihood_fn, x0, lo, hi)
    print(f"  MAP: theta={map_x[0]:.1f}  log10Lj={map_x[1]:.2f}  "
          f"G0_knot={map_x[2]:.1f}  G0_ext={map_x[3]:.1f}  ->  logL={map_ll:.2f}")

    # --- Seed initial population near MAP ---
    rng_key = jax.random.PRNGKey(42)
    rng_key, init_key, seed_key = jax.random.split(rng_key, 3)

    n_seeded = n_live // 5
    n_uniform = n_live - n_seeded
    uniform = jax.random.uniform(init_key, (n_uniform, ndim), minval=lo, maxval=hi)

    # Scatter around MAP, clipped to bounds
    spread = 0.05 * (hi - lo)
    seeded = map_x + spread * jax.random.normal(seed_key, (n_seeded, ndim))
    seeded = jnp.clip(seeded, lo, hi)

    initial_population = jnp.concatenate([uniform, seeded])
